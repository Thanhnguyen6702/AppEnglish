...
interface VocabularyDao {
    ...
    @Query("SELECT Vocabulary.* " +
            "    FROM Vocabulary " +
            "    LEFT JOIN Pronunciation " +
            "    ON Vocabulary.id = Pronunciation.id_vocab" +
            "    WHERE Pronunciation.id_vocab IS NULL ")
...
interface TopicsDAO {
    ...
    @Query(
        "SELECT Topics.id,Topics.image,Topics.topic,Theme.title,Topics.id_theme FROM Topics " +
...
interface PronunciationDao {
    ...
    fun getPronunciation(): Flow<List<PronunciationWithVocabulary>>
}